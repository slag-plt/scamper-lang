...
const templates: Messages = new Map([
  ...
  ['error-empty-pat', (_) => 'Pattern cannot be empty'],
  ...
  ['error-runtime', (vs) => `A runtime error was encountered: ${vs[0]} ${(vs as any[]).slice(1).join(' ')}`],
  ...
  ['error-var-repeated', (vs) => `Variable ${vs[0]} is repeated in the pattern`],
